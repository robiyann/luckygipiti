const fs = require('fs');
let code = fs.readFileSync('C:/Users/Administrator/Documents/bot/newgptbot/src/autopay.js', 'utf8');
...
code = code.replace(
  ...
  `    }
    ...
    if (sgProxyRaw && sgProxyRaw.includes('gw.dataimpulse.com')) {
      sgProxyRaw = sgProxyRaw.replace(/(https?:\\/\\/)([^_:]+)(?:__[^:]*)?:([^@]+)@(.+)/, (m, p1, userBase, pass, host) => {
          return \`\${p1}\${userBase}__cr.sg__session-\${sessionToken}s:\${pass}@\${host}\`;
      });
      ...
      if (!sgProxyRaw.includes('_session_')) {
        sgProxyRaw = sgProxyRaw.replace(/(https?:\\/\\/)([^:]+):([^@]+)@(.+)/, (m, p1, user, pass, host) => {
            return \`\${p1}\${user}_session_\${sessionToken}:\${pass}@\${host}\`;
        });
      }
    }
  ...
  _rotateSgProxy() {
    ...
    if (!sgProxyRaw && rawProxy && rawProxy.includes('gw.dataimpulse.com')) {
      sgProxyRaw = rawProxy.replace(/(https?:\\/\\/)([^_:]+)(?:__[^:]*)?:([^@]+)@(.+)/, (m, p1, userBase, pass, host) => {
        return \`\${p1}\${userBase}__cr.sg__session-\${newSessionToken}:\${pass}@\${host}\`;
      });
    } else if (sgProxyRaw && sgProxyRaw.includes('swiftproxy.net')) {
      if (sgProxyRaw.includes('_session_')) {
        sgProxyRaw = sgProxyRaw.replace(/_session_[^:]+:/, \`_session_\${newSessionToken}:\`);
      } else {
        sgProxyRaw = sgProxyRaw.replace(/(https?:\\/\\/)([^:]+):([^@]+)@(.+)/, (m, p1, user, pass, host) => {
          return \`\${p1}\${user}_session_\${newSessionToken}:\${pass}@\${host}\`;
        });
      }
    }
    ...
    while (attempt < 3) {
      ...
      try {
        ...
        if (is502 || isConnRefused) {
          logger.warn(this.tag + \`[\${operationName}] Proxy error (\${err.message}). Retrying (Attempt \${attempt}/3)...\`);
          if (attempt < 3) {
            this._rotateSgProxy();
            await sleep(2000);
            continue;
...
fs.writeFileSync('C:/Users/Administrator/Documents/bot/newgptbot/src/autopay.js', code);
